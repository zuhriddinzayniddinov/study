...
public class JwtTokenHandler : IJwtTokenHandler
{
    ...
    private IList<Claim> GetClaims(User user)
    {
        ...
        claims.Add(new Claim(CustomClaimNames.Permissions,
                string.Join(", ", user.Structure?.StructurePermissions.Select(x => x.Permission.Code))));

        if (user.UserCerifiticate is not null)
        {
            claims.Add(new Claim(CustomClaimNames.INN, user.UserCerifiticate.TIN));
        }
